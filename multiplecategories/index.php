...
<?php
    ...
    public function action_scatoverview($msg = '')
    {
        global $member, $oPluginAdmin;
        global $manager; //<sato(na)0.5j />
...
<?php
    ...
    public function action_scatedit($msg = '')
    {//-----
        global $member, $oPluginAdmin;
        global $manager; //<sato(na)0.5j />
...
<?php
            ...
            $this->error(_MC_SCAT_MISSING);
        }
    }//-----

    public function action_scatnew()
    {
        global $member;
    ...
    public function action_scatupdate()
    {
        global $member;
    ...
    //modify start+++++++++
    public function action_tableUpgrade()
    {
        if ($this->version == 2) {
    ...
    public function action_scatOrder()
    {
        global $member;
    ...
    public function action_scatdelete()
    {
        global $member, $oPluginAdmin;
        global $manager; //<sato(na)0.5j />
...
<?php
    ...
    public function action_scatdeleteconfirm()
    {
    ...
    public function createSubcat($name)
    {
    ...
    public function printCategoryListUD($catid, $parentid)
    {
        ...
        foreach ($this->plug->_setSubOrder() as $val) { //$val : _getSubOrder intval($row_scat->scatid)
            ...
            if ($row['catid'] == $catid) {
                $levelstr = '';
                for ($i=0; $i<$this->getDepth($val, 0); $i++) {
                    $levelstr .= "&hellip;&hellip;";
                }
    ...
    public function getDepth($scatid, $level)
    {
    ...
    public function descendantCheck($parentid, $checkid)
    {
        ...
        while ($o = sql_fetch_object($res)) {
            if ($o->scatid == $checkid) {
                ...
                if ($this->descendantCheck($o->scatid, $checkid)) {
                    return true;
                }
            }
        }
    ...
    public function updateSubcat($id, $scat)
    {
        ...
        if ($id == $scat['parentid']) {
            ...
            $branch = ($this->descendantCheck($id, $scat['parentid']) ? 'Descendant' : 'Other');
        }
...
<table style="width:auto; margin:0; padding:0;">
    ...
    public function getCategoryList($catid, $selected = 0)
    {//$selected : parentid
    ...
    public function appendChildCategory(&$categoryArray, $child)
    {
    ...
    public function printCategoryList($catid, $categoryList, $type=1, $selectedCategory=0, $def='')
    {
        $text = "";
        $catName = quickQuery('SELECT cname as result FROM '.sql_table('category').' WHERE catid='.intval($catid));
        $catName = htmlspecialchars($catName, ENT_QUOTES); //<sato(na)0.5j />
    ...
    public function walkCategoryList(&$categoryList, $catid, $type = 1, $selectedCategory = 0, $level = 0, $childrenLevel = array())
    {
        $text = "";
        if ($type == 1) {
            // Select box
    ...
    public function subcatOrd($catid, $subcatid=0)
    {
        //<sato(na)0.5j>
        $catid    = intval($catid);
    ...
    public function _getDefinedScatsFromScat($id)
    {
        $aResult = array();

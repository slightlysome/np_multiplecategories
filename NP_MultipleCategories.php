...
class NP_MultipleCategories extends NucleusPlugin
{
    ...
    {
        return 'Multiple Categories [Custom Edition]';
    }
    public function getAuthor()
    {
        return 'Anand + nakahara21 + Taka + sato(na) + shizuki + Katsumi + Leo';
    }
    ...
    //modify start+++++++++
    public function checkMSCVersion()
    {
    ...
    public function _getDefinedScats($id)
    {
        $res = sql_query(sprintf(
    ...
    public function _getScatIDs($id)
    {
        $res = sql_query(sprintf(
    ...
    public function _getCatNameFromID($id)
    {
    ...
    public function _getScatNameFromID($id)
    {
        return quickQuery(sprintf(
    ...
    public function _getScatDescFromID($id)
    {
        return quickQuery(sprintf(
    ...
    public function _getParentCatID($id)
    {
        return quickQuery(sprintf(
    ...
    public function _getScatMap($numarray)
    {
        $aResult = array();
    ...
    public function _getMultiCategories($itemid)
    {
    ...
    public function _getSubCategories($itemid)
    {
        $result = sql_query(sprintf(
    ...
    public function permuteSubcategories($subcategories)
    {
        return implode(
            ',',
            array_intersect(
...
function orderKey(key, sequence) {
    ...
    public function _getItemObject($id)
    {
        $res = sql_query(
            sprintf(
    ...
    public function showSubForm($aCategories, $itemid)
    {
    ...
    public function showForm($aCategories, $itemid)
    {
        $itemcats = array();
        $itemScats = array();
        if ($itemid !== null) { // edit item
    ...
    //<sato(na)>
    public function showFormHierarchical($parentid, $itemScats)
    {
    ...
    public function updateData($itemid, $pcatid, $selected, $s_selected)
    {
        $value = '';
        $aMulti = array();
        $aSub = array();
    ...
    {
        $query = 'DELETE FROM ' . sql_table('plug_multiple_categories') . ' WHERE item_id=' . (int)$data['itemid'];
        sql_query($query);
    }
    ...
    public function doSKinVar($skinType)
    {
        ...
        if ((int)$params[1] == 1) {
            switch ($params[2]) {
                ...
                case 'archivelist':
                    ...
                    $limit = 0;
                    if (isset($params[4]) && (int)$params[4]) {
                        $arclimit = (int)$params[4];
                    }
        ...
        $mycatid    = ($catid) ? (int)$catid : 0; //<sato(na)0.5j />
        $mysubcatid = ($subcatid) ? (int)$subcatid : 0; //<sato(na)0.5j />
        $templateName = $params[1];
        $amountEntries = 0;
        $offset = 0;
        $startpos = (int)$startpos; //<sato(na)0.5j />
        if (isset($params[2])) {
        ...
        $query .= ' LIMIT ' . (int)$startpos . ',' . (int)$amountEntries;

        $b->showUsingQuery($templateName, $query, 0, 1, 1);
    }


    public function doTemplateCommentsVar(&$item, &$comment, $what = '')
    {
        if ($what === 'itemlink') {
    ...
    public function doTemplateVar(&$item, $what = '')
    {
        ...
        if ($what === 'itemlink') {
            ...
            if ($catid) {
                ...
                if ($subcatid) {
                    $sparams[$this->getRequestName()] = (int)$subcatid; //<sato(na)0.5j />
                }
            }
    ...
    //<sato(na)0.5j>
    public function cnvHtmlUrlAttribute($forHtmlAtt__str)
    {
        //onEvent
    ...
    public function _parseItem($template, $itemid)
    {
        global $manager;
        ...
        $query .= ' FROM ' . sql_table('category') . ' as c, ' . sql_table('member') . ' as m, ' . sql_table('item') . ' as i'
            //</sato(na)0.5j>
            . ' WHERE i.iblog=' . (int)$b->getID() //<sato(na)0.5j />
            . ' and i.iauthor=m.mnumber'
            . ' and i.icat=c.catid'
            . ' and i.idraft=0'	// exclude drafts
            // don't show future items
            . ' and i.itime<=' . mysqldate($b->getCorrectTime())
            . ' and i.inumber=' . (int)$itemid
    ...
    public function showCategoryList()
    {
        ...
        $query = 'SELECT c.catid, c.cdesc as catdesc, c.cname as catname FROM ' . sql_table('category') .
            ' as c WHERE c.cblog=' . (int)$blogid . ' GROUP BY c.cname ORDER BY c.cname ASC'; //<sato(na)0.5j />
        $res = sql_query($query);
        ...
        while ($data = sql_fetch_assoc($res)) {
            ...
            $data['self'] = $CONF['Self'];
            if ($data['catid'] == (int)$catid) { //<sato(na)0.5j />
    ...
    public function showArchiveList($mode = 'month', $limit = 0)
    {
        global $CONF, $manager, $blog, $catid, $subcatid;
        ...
        $query .= ' WHERE i.iblog=' . (int)$b->getID() //<sato(na)0.5j />
            . ' and i.itime <=' . mysqldate($b->getCorrectTime())	// don't show future items!
            . ' and i.idraft=0'; // don't show draft items

        if ($catid) {
            $query .= ' and ((i.inumber=p.item_id and (p.categories REGEXP "(^|,)' . (int)$catid . '(,|$)" or i.icat=' . (int)$catid . ')) or (i.icat=' . (int)$catid . ' and p.item_id IS NULL))';
        }
        if ($subcatid) {
            $query .= ' and p.subcategories REGEXP "(^|,)' . (int)$subcatid . '(,|$)"';
        }
    ...
    //<sato(na)0.5.1j>
    public function doIf($name = '', $value = '')
    {
        global $subcatid;
